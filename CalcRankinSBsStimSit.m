function [rankb,ranks,h,meanB,meanS,groupExpb,groupExps] = CalcRankinSBsStimSit(DataSetBase,DataSetStims)
% Function which calculates the ranks order of channels in a superburst (the
% order which they fire)

% must add orders from bursts within superburst
...
for i=1:numel(vec)
    ...
    bss = DataSetStims{vec(i)}.sbs;
    bes = DataSetStims{vec(i)}.sbe;
