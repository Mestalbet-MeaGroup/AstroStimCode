...
for j=1:size(Outliers,2)
    ...
    bars=bar(sum(numtypes(1:j))+1:sum(numtypes(1:j+1))+1,[diffs(1:numtypes(j+1),j)',nan],'BarWidth',1);
...
xticks = cumsum(numtypes(1:end-1))+1;
set(gca,'TickDir','out');
...
box off; axis tight; set(gcf,'color','k');
