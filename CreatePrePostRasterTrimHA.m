function CreatePrePostRasterTrimHA(DataSetBase,DataSetStims,which)
...
tpre=DataSetBase{which}.Trim.tHA;
icpre=DataSetBase{which}.Trim.icHA;
tpost=DataSetStims{which}.Trim.tHA;
icpost=DataSetStims{which}.Trim.icHA;
if isempty(icpre)||isempty(icpost)
    display('No HA Neurons Detected');
    return;
end
a=icpre(1,:);
b=icpost(1,:);
...
for i=1:2
    ...
    c=colormap(gray(64));
    min=12; %How many minutes do you want your window?
    ...
    if i==1
        ...
        set(gca,'XTick', round(linspace(1,res,4)),'XTickLabel',round(linspace(-(min/2),0,4)),'TickDir', 'out');
